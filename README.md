...
- **抗锯齿**：线段 Xiaolin Wu 子像素混合；圆点距离场 SDF + smoothstep 边缘；绘制半径与 `point_visible` 的 NDC 半径一致（`ndc_radius_to_pixels`）。

## 题面对照（Rust 可执行版）

| 项 | 状态 |
|---|---|
| 3D→2D 透视、Y/Z 旋转、z+1.5 | 已实现 |
| 点 1/4 可见（9×9 采样） | 已实现 |
| connets 折线 + 闭合 | 已实现 |
| in-file 多题、非数字 id | 已实现 |
| CLI `-file`/`-id`/`-help`、默认最后一题 | 已实现 |
| 60 FPS | 已实现 |
| 抗锯齿 | 已实现 |
| JS/HTML 文件选择 UI | 不适用（非 Web 版） |
